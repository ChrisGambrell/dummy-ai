...
export function SchemaCard({
	...
	schema: Row<'schemas'> & { fields: Array<Row<'fields'>>; generations: Array<Row<'generations'>> }
}) {
	return (
		<Card>
			<CardHeader>
					...
					<div className='grid divide-y'>
						{schema.generations.map((gen) => (
							<div key={gen.id} className='text-sm py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-4'>
								<div>{new Date(gen.created_at).toLocaleString()}</div>
								<div>{(gen.data as Array<any>).length} item(s)</div>
